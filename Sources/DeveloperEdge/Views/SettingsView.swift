...
struct SettingsView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // Only auto-paste when the clipboard value looks like the right token type.
            // Avoids filling multiple fields from the same unrelated clipboard entry.
            let isGitHub = clip.hasPrefix("ghp_") || clip.hasPrefix("gho_") || clip.hasPrefix("github_pat_")
            let isJira   = clip.hasPrefix("ATATT")
            if store.githubToken.isEmpty,   isGitHub { githubTokenDraft = clip; editingGithubToken = true }
            if store.jiraApiToken.isEmpty,  isJira   { jiraTokenDraft   = clip; editingJiraToken   = true }
        }
    }
    ...
    private func saveGithubToken() {
        ...
        Keychain.write("githubToken", trimmed)
        Task {
    ...
    private func saveLinearKey() {
        ...
        Keychain.write("linearApiKey", trimmed)
    ...
    private func saveJiraToken() {
        ...
        Keychain.write("jiraApiToken", trimmed)
    ...
    private var repoFilterSection: some View {
        VStack(alignment: .leading, spacing: 0) {
            Text("Pin repos to show in the Services tab. Leave all off to show every repo.")
                .font(.system(size: Theme.FontSize.tinyMd))
                .foregroundColor(Theme.textMuted.opacity(0.6))
                .padding(.horizontal, 12).padding(.top, 8).padding(.bottom, 6)
            ForEach(store.services) { svc in
                let pinned = store.pinnedRepos.contains(svc.repo)
                Button(action: {
                    var updated = store.pinnedRepos
                    if pinned { updated.remove(svc.repo) } else { updated.insert(svc.repo) }
                    store.pinnedRepos = updated
                    Defaults[.pinnedRepos] = updated
                }) {
                    HStack(spacing: 10) {
                        ZStack {
                            RoundedRectangle(cornerRadius: 4)
                                .fill(pinned ? Theme.purple : Color.white.opacity(0.06))
                                .frame(width: 18, height: 18)
                            if pinned {
                                Image(systemName: "checkmark")
                                    .font(.system(size: Theme.FontSize.caption, weight: .bold))
                        ...
                        Image(systemName: svc.icon)
                            .font(.system(size: Theme.FontSize.small))
                            .foregroundColor(pinned ? Theme.purple : Theme.textMuted.opacity(0.5))
                            .frame(width: 14)
                        Text(svc.name)
                            .font(.system(size: Theme.FontSize.body, weight: .medium))
                            .foregroundColor(pinned ? Theme.textPrimary : Theme.textMuted.opacity(0.6))
                        Spacer()
                        Text(svc.repo)
                ...
                .buttonStyle(.plain)
                .background(Color.white.opacity(0.03))
                if svc.id != store.services.last?.id {
                    Rectangle().fill(Theme.divider).frame(height: 1).padding(.horizontal, 12)
                }
            }
        }

...
/// First-run guided setup: GitHub token, ticketing provider token, then clone the workspace.
struct OnboardingView: View {
    ...
    private let total = 4   // welcome(0) github(1) ticketing(2) workspace(3)
        ...
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    case 1: githubStep
                    case 2:
                        if usingLinear { linearStep } else { jiraStep }
    ...
    private var welcomeStep: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("👋")
                .font(.system(size: 40))
            Text("Welcome to \(AppConfig.current.branding.appName)")
                .font(.system(size: 20, weight: .bold))
                .foregroundColor(Theme.textPrimary)
            Text("Your menu-bar cockpit for engineering: PRs, \(usingLinear ? "Linear" : "Jira"), environments, and a one-click workspace setup. Let's get you connected in three quick steps.")
    ...
    private var linearStep: some View {
        VStack(alignment: .leading, spacing: 12) {
            stepHeader("🔗", "Connect Linear", "Shows your assigned issues in the Tasks tab.")
            link("Create a Personal API key in Linear", "https://linear.app/settings/api")
            tokenField("Paste your Linear API key (lin_api_…)", text: $linearDraft, accent: Theme.purple)
            if !store.linearApiKey.isEmpty {
                statusLine("Linear API key saved", ok: true)
            }
    ...
    private var jiraStep: some View {
        ...
        .onReceive(clipboardTimer) { _ in if !usingLinear { pollClipboardForToken() } }
    }
    ...
    private func advance() {
        if step == 1 {
            ...
            if !t.isEmpty { store.githubToken = t; Keychain.write("githubToken", t) }
        }
        if step == 2 {
            if usingLinear {
                let t = linearDraft.trimmingCharacters(in: .whitespacesAndNewlines)
                if !t.isEmpty { store.linearApiKey = t; Keychain.write("linearApiKey", t) }
            } else {
                let t = jiraDraft.trimmingCharacters(in: .whitespacesAndNewlines)
                if !t.isEmpty {
                    store.jiraApiToken = t; Keychain.write("jiraApiToken", t)
                    store.fetchJiraDisplayName()
                }
            }
        }

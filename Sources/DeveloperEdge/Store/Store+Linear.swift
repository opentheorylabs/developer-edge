...
extension Store {
    func fetchLinearIssues() {
        ...
        if let teamId = AppConfig.current.linear?.teamId, !teamId.isEmpty {
            teamFilter = #", filter: { team: { key: { eq: "\#(teamId)" } } }"#
        } else {
            teamFilter = ""
        }

        let query = """
        { "query": "{ viewer { assignedIssues(orderBy: updatedAt\(teamFilter)) { nodes { id identifier title state { name type } priority priorityLabel url team { name } } } } }" }
        """

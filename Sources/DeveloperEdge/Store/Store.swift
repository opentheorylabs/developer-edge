...
final class Store: ObservableObject {
    ...
    /// Repos shown in the Services tab CI column. Empty = show all.
    @Published var pinnedRepos: Set<String>        = Defaults[.pinnedRepos]
    ...
    // MARK: - Init
    // Keychain reads are synchronous but fast (< 1 ms for local login keychain).
    // Stored-property initializers can't call static methods with side effects,
    // so we load credentials in init() instead.
    init() {
        githubToken  = Keychain.migratingRead("githubToken",  legacy: .githubToken)
        jiraApiToken = Keychain.migratingRead("jiraApiToken", legacy: .jiraApiToken)
        linearApiKey = Keychain.migratingRead("linearApiKey", legacy: .linearApiKey)
    }
